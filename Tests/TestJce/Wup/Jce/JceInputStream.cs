...
namespace Wup.Jce
{
    ...
    public class JceInputStream
    {
        ...
        //private JceInputStream(MemoryStream ms)
        //{
        //    this.ms = m;s
        //    br = null;
        //    br = new BinaryReader(ms);
        ...
        //public void warp(byte[] bs, int index = 0)
        //{
        //    if (null != this.ms)
        //    {
        //        ms = null;
...
*/
        ...
        */
        ...
        public JceStruct Read(JceStruct o, int tag, bool isRequire)
        {
            ...
            //如果表格数据和协议不匹配，则默认返回对应类型的构造实例，不返回null
            if (reff == null)
            {
                //Diagnostic.Fatal("JceInputSream ParseError,tag cannot be found!!!!!!!!!!!!!! {0} == null", o.GetType().Name);
                try
                {
                    // 必须重新创建一个，否则，会导致在同一个对象上赋值，这是由于C#的引用引起的
                    reff = (JceStruct)BasicClassTypeUtil.CreateObject(o.GetType());
